    ...
    <div className="min-h-screen bg-[#030712] text-slate-300 font-sans selection:bg-cyan-500 relative">
      ...
      <div className="max-w-6xl mx-auto p-6 md:p-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* NAVEGACIÓN DEL AULA INVERTIDA (Sidebar) */}
          <div className="space-y-4">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
               <p className="text-[10px] text-slate-500 leading-relaxed text-justify">
                 * El modelo de aula invertida requiere que el cursante domine el material teórico en esta plataforma antes de proceder a la evaluación centrada en la resolución de casos reales.
               </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-[#0a1120]/80 border border-slate-800 rounded-xl p-8 backdrop-blur-sm min-h-[500px]">
              ...
              {moduloActivo === 3 && (
                <div className="animate-fade-in-up flex flex-col items-center justify-center min-h-[400px] text-center">
                  ...
                  <h2 className="text-2xl font-black text-white mb-2">Simulador de Casos Prácticos</h2>
                  <p className="text-sm text-slate-400 max-w-md mb-8">
                    La evaluación se centrará en tu capacidad para resolver problemas bajo el modelo de competencias. Se te presentará un escenario real extraído de nuestro historial operativo.
                  </p>
                  
                  {!evaluacionIniciada ? (
                    <button onClick={() => setEvaluacionIniciada(true)} className="px-8 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold tracking-widest rounded transition-all shadow-[0_0_20px_rgba(16,185,129,0.4)]">
                      INICIAR EVALUACIÓN
                    </button>
                  ) : (
                    <div className="w-full text-left bg-[#030712] border border-cyan-900/50 p-6 rounded">
                       <p className="font-mono text-cyan-400 text-xs mb-2">CASO #01 - INTERVENCIÓN EN DOMICILIO</p>
                       <p className="text-sm text-slate-300 mb-6">Un cliente reporta un cortocircuito en su placa base. Al llegar, el equipo aún está conectado a la red eléctrica pero emite olor a quemado. Según los protocolos de intervención aprendidos, ¿cuál es el primer paso táctico a seguir?</p>
                       
                       <div className="space-y-3">
                         <button className="w-full text-left p-3 border border-slate-700 rounded text-sm text-slate-400 hover:bg-slate-800 transition-colors">A) Abrir el chasis inmediatamente para ventilar.</button>
                         <button className="w-full text-left p-3 border border-emerald-700/50 bg-emerald-950/20 rounded text-sm text-emerald-400 transition-colors">B) Aislar la fuente de energía y asegurar el perímetro antes del contacto físico.</button>
                         <button className="w-full text-left p-3 border border-slate-700 rounded text-sm text-slate-400 hover:bg-slate-800 transition-colors">C) Solicitar al cliente que encienda el equipo para replicar la falla.</button>
                       </div>
                    </div>
                  )}
                </div>
