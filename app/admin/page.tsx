...
export default function AdminDashboard() {
  ...
  // === CORRECCIÓN: RESTAURACIÓN DE VARIABLES DE ANALÍTICA Y FILTRO ===
  ...
  const generarPDFIngreso = async (cita: any) => {
    ...
    // CORRECCIÓN: Se eliminó el parámetro letterSpacing que causaba el error TS2353
    doc.text("SISTEMA DE DESPLIEGUE Y TELEMÁTICA", 105, 29, { align: "center" });
    ...
    doc.roundedRect(14, 60, 182, 25, 2, 2, 'S'); 
    doc.setFillColor(15, 23, 42); 
    doc.roundedRect(14, 60, 182, 8, 2, 2, 'F'); 
    doc.setTextColor(34, 211, 238); doc.setFontSize(9);
    doc.text("PARÁMETROS DE DESPLIEGUE", 18, 65.5);
    
    doc.setTextColor(0, 0, 0); doc.setFontSize(9);
    doc.setFont("helvetica", "bold"); doc.text("Solicitante:", 18, 75); doc.setFont("helvetica", "normal"); doc.text(cita.nombre, 40, 75);
    doc.setFont("helvetica", "bold"); doc.text("Contacto:", 110, 75); doc.setFont("helvetica", "normal"); doc.text(telLimpio, 130, 75);
    doc.setFont("helvetica", "bold"); doc.text("Ubicación:", 18, 81); 
    let dirBreve = cita.direccion || "No especificada";
    if (dirBreve.length > 80) dirBreve = dirBreve.substring(0, 80) + "...";
    doc.setFont("helvetica", "normal"); doc.text(dirBreve, 40, 81);
    ...
    // 5. SECCIÓN DE FIRMAS OFICIALES (ELIMINADA LA PALABRA LABORATORIO)
    ...
    doc.text("Miguel Angel Cuenca", 155, 255, { align: "center" });
    doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(100);
    doc.text("Firma Autorizada", 155, 260, { align: "center" });
  ...
  const generarPDFEntrega = async (cita: any) => {
    ...
    // 1. CABECERA FUTURISTA DE FINALIZACIÓN
    doc.setFillColor(8, 15, 30); 
    doc.rect(0, 0, 210, 48, 'F');
    ...
    doc.setFont("helvetica", "normal");
    // CORRECCIÓN: Se eliminó el parámetro letterSpacing que causaba el error TS2353
    doc.text("CERTIFICADO DE FINALIZACIÓN OPERATIVA", 105, 29, { align: "center" });
    ...
    doc.setFont("helvetica", "bold"); doc.text(`CLIENTE:`, 14, 62); doc.setFont("helvetica", "normal"); doc.text(cita.nombre, 45, 62);
    doc.setFont("helvetica", "bold"); doc.text(`CONTACTO:`, 14, 69); doc.setFont("helvetica", "normal"); doc.text(telLimpio, 45, 69);
    doc.setFont("helvetica", "bold"); doc.text(`FECHA CIERRE:`, 115, 62); doc.setFont("helvetica", "normal"); doc.text(new Date().toLocaleDateString(), 155, 62);
    doc.setFont("helvetica", "bold"); doc.text(`MODALIDAD:`, 115, 69); doc.setFont("helvetica", "normal"); doc.text(cita.modalidad.toUpperCase(), 155, 69);
    ...
    doc.setFont("helvetica", "bold"); doc.text(`Adelanto Registrado:`, 18, finalY + 25); doc.setFont("helvetica", "normal"); doc.text(`- ${adelanto.toFixed(2)} Bs.`, 160, finalY + 25, { align: "right" });
    ...
    // 5. VALIDACIÓN Y FIRMAS (SIN LABORATORIO)
    ...
    if (cita.modalidad === "En Domicilio") {
      ...
      doc.text("Firma del Cliente", 55, 255, { align: "center" }); 
      doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(100);
      doc.text(`Conformidad de Recepción`, 55, 260, { align: "center" }); 
    }
                ...
                <table className="w-full text-left border-collapse min-w-[1000px]">
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    {loading ? ( 
                      ...
                      // CORRECCIÓN: (cita: any) soluciona el error TS7006
